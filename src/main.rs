...
fn default_connecting_server_arg(
    ...
    app_setup::default_connecting_server_arg(initial_state, preferred_realm)
}
...
fn configure_terrain_texture_isolation(app: &mut App, args: &[String]) {
    if has_flag(args, "--no-terrain-textures") {
        app.world_mut()
            .resource_mut::<terrain::AdtManager>()
...
fn configure_water_and_skybox_isolation(app: &mut App, args: &[String]) {
    if has_flag(args, "--no-terrain-water") {
        app.world_mut()
            .resource_mut::<terrain::AdtManager>()
            .load_water = false;
    }
    if has_flag(args, "--no-skybox") {
        app.insert_resource(sky::SkyboxVisualsDisabled);
    }
}
