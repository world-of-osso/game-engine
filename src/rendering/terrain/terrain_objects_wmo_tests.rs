...
// --- WMO liquid surface: additional tests ---
...
fn wmo_liquid_tile_exists_marks_0x0f_as_empty() {
    ...
    assert!(!wmo_liquid_tile_exists(&liquid, 0, 1)); // 0x0F = empty
}
...
fn wmo_liquid_geometry_normals_up() {
    ...
    // All normals should point up (Y in Bevy)
    for n in &normals {
...
fn wmo_liquid_geometry_index_pattern() {
    ...
    // One tile = 2 triangles = 6 indices
