...
impl Plugin for FriendsPlugin {
    fn build(&self, app: &mut App) {
        ...
        register_outgoing_handler(app, send_pending_actions, |world| {
            !world
                .resource::<FriendsRuntimeState>()
                .pending_actions
                .is_empty()
        });
        register_message_handler::<FriendsStateUpdate, _>(app, receive_friends_updates, |_| true);
    }
}
...
struct FriendsSenders<'w, 's> {
    add: MessageSenders<'w, 's, AddFriend>,
    remove: MessageSenders<'w, 's, RemoveFriend>,
    presence: MessageSenders<'w, 's, SetPresenceStatus>,
}
...
fn receive_friends_updates(
    ...
    mut receivers: MessageReceivers<FriendsStateUpdate>,
) {
...
mod tests {
    ...
    fn queued_requests_wait_for_dispatch_then_report_disconnection() {
        ...
        app.add_plugins(FriendsPlugin)
            .init_resource::<FriendsStatusSnapshot>();
        let (reply, responses) = mpsc::channel();
        {
            let mut runtime = app.world_mut().resource_mut::<FriendsRuntimeState>();
        ...
        for _ in 0..2 {
            ...
            assert_eq!(error, "friends are unavailable: not connected");
        }
        let runtime = app.world().resource::<FriendsRuntimeState>();
    ...
    fn idle_dispatch_leaves_waiting_replies_untouched() {
        ...
        let (reply, responses) = mpsc::channel();
        app.world_mut()
            .resource_mut::<FriendsRuntimeState>()
            .pending_replies
            .push_back(reply);
