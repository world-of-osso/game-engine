...
pub(crate) fn movement_to_direction(
    ...
    match movement.direction {
        MoveDirection::Forward => { dir[0] += forward[0]; dir[2] += forward[2]; }
        MoveDirection::Backward => { dir[0] -= forward[0]; dir[2] -= forward[2]; }
        MoveDirection::Left => { dir[0] -= right[0]; dir[2] -= right[2]; }
        MoveDirection::Right => { dir[0] += right[0]; dir[2] += right[2]; }
        MoveDirection::None => {}
    }
    dir
}
...
pub(crate) fn spawn_replicated_player(
    ...
    let Ok((pos, player, rotation)) = query.get(entity) else { return };
    let is_local = is_local_player_entity(&player.name, selected.as_deref());
    info!("Spawning replicated player '{}' (local={is_local}) at ({:.1}, {:.1}, {:.1})", player.name, pos.x, pos.y, pos.z);
    let position = crate::networking::net_position_to_bevy(pos);
    let yaw = rotation.map_or(std::f32::consts::PI, |r| r.y);
    commands.entity(entity).insert((
        Transform::from_translation(position).with_rotation(Quat::from_rotation_y(yaw)),
        Visibility::default(), ReplicatedVisualEntity, RemoteEntity,
        InterpolationTarget { target: position }, RotationTarget { yaw },
    ));
    attach_player_model(
        &mut commands, &mut meshes, &mut materials, &mut effect_materials,
        &mut images, &mut inv_bp, &creature_display_map, entity, player, is_local,
    );
}
...
fn attach_player_model(
    ...
    let model_spawned = try_spawn_player_m2(
        commands, meshes, materials, effect_materials, images, inv_bp,
        creature_display_map, entity, player,
    );
    if !model_spawned {
...
fn try_spawn_player_m2(
    ...
    let Some(model_path) = resolve_player_model_path(player) else { return false };
    let spawned = crate::m2_scene::spawn_full_m2_on_entity(
        commands, meshes, materials, effect_materials, images, inv_bp,
        &model_path, creature_display_map, entity,
    );
    if spawned {
...
pub(crate) fn sync_replicated_player_customization(
    ...
    player_query: Query<
        (Entity, &NetPlayer, Option<&NetEquipmentAppearance>, Option<&AppliedPlayerAppearance>, Option<&Children>),
    ...
    material_query: Query<(Entity, &MeshMaterial3d<StandardMaterial>, Option<&crate::m2_spawn::GeosetMesh>, Option<&crate::m2_spawn::BatchTextureType>, &ChildOf)>,
    ...
    for (entity, player, equipment_appearance, applied, children) in &player_query {
        ...
        if children.is_none_or(|c| c.is_empty()) { continue; }
        apply_player_customization_for_entity(
            &mut commands, entity, selection, equipment_snapshot,
            &customization_db, &char_tex, &outfit_data,
            &parent_query, &geoset_query, &mut visibility_query, &equipment_item_query,
            &material_query, &mut equipment_query, &mut images, &mut materials,
        );
    }
}
...
fn apply_player_customization_for_entity(
    ...
    let resolved_equipment = equipment_appearance::resolve_equipment_appearance(
        &equipment_snapshot, outfit_data, selection.race, selection.sex,
    );
    crate::character_customization::apply_character_customization(
        selection, customization_db, char_tex, Some(&resolved_equipment),
        entity, images, materials, parent_query, geoset_query,
        visibility_query, equipment_item_query, material_query,
    );
    ...
    commands.entity(entity).insert(AppliedPlayerAppearance { selection, equipment: equipment_snapshot });
}
...
pub(crate) fn tag_local_player(
    ...
    if players.iter().any(|(_, p, local)| local && p.name == *name) { return; }
    let (chosen, match_count) =
        choose_local_player_entity(name, players.iter().map(|(e, p, _)| (e, p)));
    if match_count > 1 {
        warn!("Found {match_count} replicated players named '{}'; choosing newest entity as local", name);
    }
...
fn apply_local_player_tag(
    ...
    if should_be_local && !is_local {
        ...
        commands.entity(entity).insert((
            LocalPlayer, Player, MovementState::default(),
            CharacterFacing::default(), crate::collision::CharacterPhysics::default(),
        ));
    } else if !should_be_local && is_local {
        commands.entity(entity).remove::<(
            LocalPlayer, Player, MovementState, CharacterFacing, crate::collision::CharacterPhysics,
        )>();
    }
}
