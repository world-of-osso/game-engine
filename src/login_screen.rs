...
fn build_login_ui(
    ...
    commands.insert_resource(LoginUi {
        root, server_input, username_input, password_input, connect_button, reconnect_button,
        create_account_button, menu_button, save_checkbox, exit_button, status_text,
    });
}
...
fn teardown_login_ui(mut ui: ResMut<UiState>, login_ui: Option<Res<LoginUi>>, mut commands: Commands) {
...
fn login_mouse_input(
    ...
    handle_mouse_click(&mut ui, login, cursor, &mut focus, &mut next_state,
        &mut status, &mut login_mode, &mut save_account, &mut commands, &mut exit);
}

fn handle_mouse_click(
    ui: &mut UiState, login: &LoginUi, cursor: Vec2,
    focus: &mut LoginFocus, next_state: &mut NextState<GameState>,
    status: &mut LoginStatus, login_mode: &mut networking::LoginMode,
    save_account: &mut LoginSaveAccount, commands: &mut Commands,
    ...
    if hit_frame(ui, login.server_input, cx, cy) {
        ...
        handle_button_click(ui, login, cx, cy, focus, next_state, status, login_mode, save_account, commands, exit);
    }
}

#[allow(clippy::too_many_arguments)]
fn handle_button_click(
    ui: &mut UiState, login: &LoginUi, cx: f32, cy: f32,
    focus: &mut LoginFocus, next_state: &mut NextState<GameState>,
    status: &mut LoginStatus, login_mode: &mut networking::LoginMode,
    save_account: &mut LoginSaveAccount, commands: &mut Commands,
    exit: &mut MessageWriter<AppExit>,
    ...
    if hit_frame(ui, login.connect_button, cx, cy) {
        if let Some(WidgetData::Button(bd)) = ui.registry.get_mut(login.connect_button).and_then(|f| f.widget_data.as_mut()) {
            bd.state = BtnState::Pushed;
        }
        try_connect(&ui.registry, login, status, next_state, &*login_mode, commands);
...
fn login_keyboard_input(
    ...
    for event in key_events.read() {
        ...
        if let Key::Character(ch) = &event.logical_key {
            ...
            handle_login_key(event.key_code, focused_id, &mut ui, login,
...
fn handle_login_key(
    key: KeyCode, focused_id: u64, ui: &mut UiState, login: &LoginUi,
    status: &mut LoginStatus, next_state: &mut NextState<GameState>,
    mode: &networking::LoginMode, commands: &mut Commands,
) {
    match key {
...
fn create_check_button(reg: &mut FrameRegistry, name: &str, parent: Option<u64>, w: f32, h: f32, checked: bool) -> u64 {
...
fn set_layout(reg: &mut FrameRegistry, id: u64, x: f32, y: f32, w: f32, h: f32) {
    let (relative_to, x_offset, y_offset) = reg
        ...
        .unwrap_or((None, x, y));
    if let Some(frame) = reg.get_mut(id) { frame.width = w; frame.height = h; frame.layout_rect = None; }
...
fn set_button_textures(reg: &mut FrameRegistry, id: u64, normal: &str, pushed: &str, highlight: &str) {
    if let Some(WidgetData::Button(bd)) = reg.get_mut(id).and_then(|f| f.widget_data.as_mut()) {
