...
use crate::status::{
    ...
    GroupStatusSnapshot, InventoryItemEntry, InventorySearchSnapshot,
...
pub fn dispatch_status_request(cmd: &Command, ctx: &DispatchContext) -> bool {
    let text = match &cmd.request {
        ...
        Request::ReputationsStatus | Request::ReputationList => format_reputations_status(ctx.reputations_status),
        ...
        Request::GuildVaultStatus => format_storage_list("guild_vault", &ctx.guild_vault_status.entries),
        ...
        Request::CombatRecap { target } => format_combat_recap(ctx.combat_log_status, target.as_deref()),
        Request::CollectionMounts { missing } => format_collection_mounts(ctx.collection_status, *missing),
        Request::CollectionPets { missing } => format_collection_pets(ctx.collection_status, *missing),
        Request::ProfessionRecipes { text } => format_profession_recipes(ctx.profession_status, text),
        _ => return false,
    };
...
pub fn format_network_status(snapshot: &NetworkStatusSnapshot, connected: bool) -> String {
    format!(
        ...
        snapshot.local_client_id.map(|id| id.to_string()).unwrap_or_else(|| "-".into()),
        snapshot.zone_id,
...
pub fn format_terrain_status(snapshot: &TerrainStatusSnapshot) -> String {
    format!(
        ...
        if snapshot.map_name.is_empty() { "-" } else { &snapshot.map_name },
        snapshot.initial_tile.0, snapshot.initial_tile.1,
        snapshot.load_radius, snapshot.loaded_tiles, snapshot.pending_tiles,
        snapshot.failed_tiles, snapshot.server_requested_tiles, snapshot.heightmap_tiles,
    )
}
...
pub fn format_sound_status(snapshot: &SoundStatusSnapshot) -> String {
    format!(
        "enabled: {}\nmuted: {}\nmaster_volume: {:.2}\nfootstep_volume: {:.2}\nambient_volume: {:.2}\nambient_entities: {}\nactive_sinks: {}",
        snapshot.enabled, snapshot.muted, snapshot.master_volume,
        snapshot.footstep_volume, snapshot.ambient_volume,
        snapshot.ambient_entities, snapshot.active_sinks,
    )
}
...
pub fn format_currencies_status(snapshot: &CurrenciesStatusSnapshot) -> String {
    ...
    let lines = snapshot.entries.iter()
...
pub fn format_reputations_status(snapshot: &ReputationsStatusSnapshot) -> String {
    ...
    let lines = snapshot.entries.iter()
        .map(|e| format!("{} {} standing={} value={}", e.faction_id, e.faction_name, e.standing, e.value))
        .collect::<Vec<_>>().join("\n");
    format!("reputations: {}\n{lines}", snapshot.entries.len())
}
...
fn opt_float0(value: Option<f32>) -> String {
    value.map(|v| format!("{v:.0}")).unwrap_or_else(|| "-".into())
}
...
pub fn format_character_stats_status(snapshot: &CharacterStatsSnapshot) -> String {
    format!(
        ...
        opt_int(snapshot.level), opt_int(snapshot.race), opt_int(snapshot.class),
        opt_float0(snapshot.health_current), opt_float0(snapshot.health_max),
        opt_float0(snapshot.mana_current), opt_float0(snapshot.mana_max),
        opt_float2(snapshot.movement_speed), snapshot.zone_id,
    )
}
...
pub fn format_bags_status(snapshot: Option<&AuctionInventorySnapshot>) -> String {
    ...
    let lines = snapshot.items.iter().enumerate()
        .map(|(slot, item)| format!("{} {} {} x{} q{} lvl{}", slot, item.item_guid, item.name, item.stack_count, item.quality, item.required_level))
        .collect::<Vec<_>>().join("\n");
...
pub fn format_storage_list(title: &str, entries: &[crate::status::StorageItemEntry]) -> String {
    ...
    let lines = entries.iter()
        .map(|e| format!("{} {} {} {} x{}", e.slot, e.item_guid, e.item_id, e.name, e.stack_count))
        .collect::<Vec<_>>().join("\n");
    format!("{title}: {}\n{lines}", entries.len())
}
...
pub fn format_item_info(item: &crate::item_info::ItemStaticInfo, appearance_known: bool) -> String {
    format!(
        "item_id: {}\nname: {}\nquality: {}\nitem_level: {}\nrequired_level: {}\ninventory_type: {}\nsell_price: {}\nstackable: {}\nbonding: {}\nexpansion_id: {}\nappearance_known: {}",
        item.item_id, item.name, item.quality, item.item_level, item.required_level,
        item.inventory_type, item.sell_price, item.stackable, item.bonding,
        item.expansion_id, appearance_known,
    )
}
...
pub fn format_quest_list(snapshot: &QuestLogStatusSnapshot) -> String {
    ...
    let lines = snapshot.entries.iter()
        .map(|e| format!("{} {} zone={} repeat={} completed={} objectives={}", e.quest_id, e.title, e.zone, quest_repeatability_label(&e.repeatability), e.completed, e.objectives.len()))
        .collect::<Vec<_>>().join("\n");
    format!("quests: {}\n{lines}", snapshot.entries.len())
}

pub fn format_quest_watch(snapshot: &QuestLogStatusSnapshot) -> String {
    let watched = snapshot.watched_quest_ids.iter()
        .filter_map(|id| snapshot.entries.iter().find(|e| e.quest_id == *id))
        .collect::<Vec<_>>();
    if watched.is_empty() {
        return "quest_watch: 0\n-".into();
    }
    let lines = watched.iter()
        .map(|e| {
            let obj = e.objectives.iter().map(|o| format!("{} {}/{}", o.text, o.current, o.required)).collect::<Vec<_>>().join("; ");
...
pub fn format_quest_show(snapshot: &QuestLogStatusSnapshot, quest_id: u32) -> String {
    ...
    let objectives = if entry.objectives.is_empty() {
        ...
        entry.objectives.iter()
            .map(|o| format!("{} {}/{} completed={}", o.text, o.current, o.required, o.completed))
            .collect::<Vec<_>>().join("\n")
    };
    format!(
        "quest_id: {}\ntitle: {}\nzone: {}\nrepeatability: {}\ncompleted: {}\nobjectives:\n{}",
        entry.quest_id, entry.title, entry.zone, quest_repeatability_label(&entry.repeatability), entry.completed, objectives,
    )
}
...
pub fn format_group_roster(snapshot: &GroupStatusSnapshot) -> String {
    ...
    let lines = snapshot.members.iter()
        .map(|m| format!("{} leader={} role={} online={} subgroup={}", m.name, m.is_leader, group_role_label(&m.role), m.online, m.subgroup))
        .collect::<Vec<_>>().join("\n");
...
pub fn format_group_status(snapshot: &GroupStatusSnapshot) -> String {
    format!(
        "in_group: {}\nis_raid: {}\nmembers: {}\nready: {}/{}\nlast_message: {}",
        !snapshot.members.is_empty(), snapshot.is_raid, snapshot.members.len(),
        snapshot.ready_count, snapshot.total_count,
...
pub fn format_combat_log(snapshot: &CombatLogStatusSnapshot, lines: u16) -> String {
    ...
    let selected = snapshot.entries.iter().rev().take(take_count).collect::<Vec<_>>();
    let text = selected.iter().map(|e| format_combat_entry(e)).collect::<Vec<_>>().join("\n");
    format!("combat_log: {}\n{}", selected.len(), text)
}
...
pub fn format_combat_recap(snapshot: &CombatLogStatusSnapshot, target: Option<&str>) -> String {
    ...
    let filtered = snapshot.entries.iter().rev()
        .filter(|e| target == "current" || e.target.to_ascii_lowercase() == target || e.source.to_ascii_lowercase() == target)
        .take(10).collect::<Vec<_>>();
    if filtered.is_empty() {
        return format!("combat_recap target={target}: 0\n-");
    }
    let text = filtered.iter().map(|e| format_combat_entry(e)).collect::<Vec<_>>().join("\n");
...
fn format_combat_entry(entry: &CombatLogEntry) -> String {
    ...
    format!(
        ...
        kind, entry.source, entry.target,
        entry.spell.as_deref().unwrap_or("-"),
        entry.amount.map(|v| v.to_string()).unwrap_or_else(|| "-".into()),
...
pub fn format_collection_mounts(snapshot: &CollectionStatusSnapshot, missing: bool) -> String {
    let filtered = snapshot.mounts.iter().filter(|e| !missing || !e.known).collect::<Vec<_>>();
    if filtered.is_empty() {
        return "mounts: 0\n-".into();
    }
    let lines = filtered.iter().map(|e| format!("{} {} known={}", e.mount_id, e.name, e.known)).collect::<Vec<_>>().join("\n");
    format!("mounts: {}\n{lines}", filtered.len())
}

pub fn format_collection_pets(snapshot: &CollectionStatusSnapshot, missing: bool) -> String {
    let filtered = snapshot.pets.iter().filter(|e| !missing || !e.known).collect::<Vec<_>>();
    if filtered.is_empty() {
        return "pets: 0\n-".into();
    }
    let lines = filtered.iter().map(|e| format!("{} {} known={}", e.pet_id, e.name, e.known)).collect::<Vec<_>>().join("\n");
    format!("pets: {}\n{lines}", filtered.len())
}
...
pub fn format_profession_recipes(snapshot: &ProfessionStatusSnapshot, text: &str) -> String {
    ...
    let filtered = snapshot.recipes.iter()
        .filter(|e| needle.is_empty() || e.name.to_ascii_lowercase().contains(&needle))
        .collect::<Vec<_>>();
    if filtered.is_empty() {
        return format!("recipes text={text}: 0\n-");
    }
    let lines = filtered.iter()
        .map(|e| format!("{} {} profession={} craftable={} cooldown={}", e.spell_id, e.name, e.profession, e.craftable, e.cooldown.as_deref().unwrap_or("-")))
        .collect::<Vec<_>>().join("\n");
    format!("recipes text={text}: {}\n{lines}", filtered.len())
}

pub fn format_map_position(snapshot: &MapStatusSnapshot) -> String {
    let waypoint = snapshot.waypoint.map(|w| format!("{:.2},{:.2}", w.x, w.y)).unwrap_or_else(|| "-".into());
    format!("zone_id: {}\nposition: {:.2},{:.2}\nwaypoint: {}", snapshot.zone_id, snapshot.player_x, snapshot.player_z, waypoint)
}
...
pub fn format_map_target(
    ...
    format!(
        ...
        entity.to_bits(), transform.translation.x, transform.translation.z, distance,
    )
}
    ...
    if let Some(snapshot) = bags {
        for (index, item) in snapshot.items.iter().enumerate() {
            entries.push(InventoryItemEntry {
                storage: "bags".into(), slot: index as u32,
                item_guid: item.item_guid, item_id: item.item_id,
                name: item.name.clone(), stack_count: item.stack_count,
            });
        }
    }
    entries.extend(guild_vault.iter().map(|e| InventoryItemEntry {
        storage: "guild_vault".into(), slot: e.slot, item_guid: e.item_guid,
        item_id: e.item_id, name: e.name.clone(), stack_count: e.stack_count,
    }));
    entries.extend(warbank.iter().map(|e| InventoryItemEntry {
        storage: "warbank".into(), slot: e.slot, item_guid: e.item_guid,
        item_id: e.item_id, name: e.name.clone(), stack_count: e.stack_count,
    }));
    entries
}

pub fn inventory_search_snapshot(entries: &[InventoryItemEntry], text: &str) -> InventorySearchSnapshot {
    let needle = text.trim().to_ascii_lowercase();
    if needle.is_empty() {
        return InventorySearchSnapshot { entries: entries.to_vec() };
    }
    InventorySearchSnapshot {
        entries: entries.iter().filter(|e| e.name.to_ascii_lowercase().contains(&needle)).cloned().collect(),
    }
}
...
pub fn format_inventory_list(entries: &[InventoryItemEntry]) -> String {
    ...
    let lines = entries.iter()
        .map(|e| format!("{}:{} {} {} {} x{}", e.storage, e.slot, e.item_guid, e.item_id, e.name, e.stack_count))
        .collect::<Vec<_>>().join("\n");
    format!("inventory: {}\n{lines}", entries.len())
}
...
pub fn format_inventory_search(snapshot: &InventorySearchSnapshot, text: &str) -> String {
    ...
    for storage in ["bags", "guild_vault", "warbank"] {
        let grouped = snapshot.entries.iter().filter(|e| e.storage == storage).collect::<Vec<_>>();
        if grouped.is_empty() { continue; }
        lines.push(format!("[{storage}]"));
        lines.extend(grouped.iter().map(|e| format!("{} {} {} {} x{}", e.slot, e.item_guid, e.item_id, e.name, e.stack_count)));
    }
    format!("inventory search text={text}: {}\n{}", snapshot.entries.len(), lines.join("\n"))
}

pub fn format_inventory_whereis(entries: &[InventoryItemEntry], item_id: u32) -> String {
    let matches = entries.iter().filter(|e| e.item_id == item_id).collect::<Vec<_>>();
    if matches.is_empty() {
        return format!("inventory whereis item_id={item_id}: 0\n-");
    }
    let lines = matches.iter()
        .map(|e| format!("{}:{} {} {} x{}", e.storage, e.slot, e.item_guid, e.name, e.stack_count))
        .collect::<Vec<_>>().join("\n");
    format!("inventory whereis item_id={item_id}: {}\n{lines}", matches.len())
}
...
pub fn resolve_spell_identifier(spell: &str) -> Result<(Option<u32>, String), String> {
    ...
    let valid_token = trimmed.chars().all(|ch| ch.is_ascii_alphanumeric() || ch == '_' || ch == ' ' || ch == '-');
    if !valid_token {
...
pub fn resolve_spell_target(selector: Option<&str>, current_target: &CurrentTarget) -> Result<Option<u64>, String> {
    let Some(selector) = selector else {
        return current_target.0.map(|e| Some(e.to_bits())).ok_or_else(|| "no current target selected".into());
    };
    ...
    selector.parse::<u64>().map(Some).map_err(|_| format!("invalid target selector '{selector}'"))
}
...
mod tests {
    use super::*;
    use crate::status::{
        CombatLogEntry, CombatLogEventKind, CombatLogStatusSnapshot, CollectionStatusSnapshot,
    ...
    fn formats_sound_status_snapshot() {
        let text = format_sound_status(&SoundStatusSnapshot {
            enabled: true, muted: false, master_volume: 0.8,
            footstep_volume: 0.5, ambient_volume: 0.3, ambient_entities: 1, active_sinks: 2,
        });
    ...
    fn formats_character_stats_status_snapshot() {
        let text = format_character_stats_status(&crate::status::CharacterStatsSnapshot {
            name: Some("Thrall".into()), level: Some(12), race: Some(2), class: Some(7),
            health_current: Some(120.0), health_max: Some(150.0),
            mana_current: Some(80.0), mana_max: Some(100.0),
            movement_speed: Some(7.0), zone_id: 12,
        });
    ...
    fn formats_item_info_with_appearance_state() {
        let text = format_item_info(
            &crate::item_info::ItemStaticInfo {
                item_id: 2589, name: "Linen Cloth".into(), quality: 1, item_level: 5,
                required_level: 1, inventory_type: 0, sell_price: 13, stackable: 200,
                bonding: 0, expansion_id: 0,
            },
            true,
        );
    ...
    fn inventory_search_groups_entries_by_storage() {
        let snapshot = InventorySearchSnapshot {
            entries: vec![
                InventoryItemEntry { storage: "bags".into(), slot: 4, item_guid: 101, item_id: 25, name: "Worn Shortsword".into(), stack_count: 1 },
                InventoryItemEntry { storage: "guild_vault".into(), slot: 7, item_guid: 202, item_id: 2589, name: "Linen Cloth".into(), stack_count: 12 },
            ],
        };
        ...
        let entries = build_inventory_entries(
            Some(&AuctionInventorySnapshot {
                ...
                items: vec![AuctionInventoryItem {
                    item_guid: 42, item_id: 2589, name: "Linen Cloth".into(),
                    quality: 1, required_level: 1, stack_count: 7, vendor_sell_price: 13,
                }],
            }),
            &[], &[],
        );
    ...
    fn quest_list_formats_daily_and_objective_counters() {
        let snapshot = QuestLogStatusSnapshot {
            entries: vec![crate::status::QuestEntry {
                quest_id: 101, title: "Defend the Farm".into(), zone: "Westfall".into(),
                completed: false, repeatability: QuestRepeatability::Daily,
                objectives: vec![crate::status::QuestObjectiveEntry {
                    text: "Harvest Watchers slain".into(), current: 3, required: 8, completed: false,
                }],
            }],
    ...
    fn group_roster_formatter_shows_leader_role_online_and_subgroup() {
        let snapshot = GroupStatusSnapshot {
            ...
            members: vec![crate::status::GroupMemberEntry {
                name: "Thrall".into(), role: GroupRole::Healer,
                is_leader: true, online: true, subgroup: 1,
            }],
            ready_count: 1, total_count: 1, last_server_message: None,
        };
    ...
    fn make_combat_entry(kind: CombatLogEventKind, text: &str) -> CombatLogEntry {
        CombatLogEntry { kind, source: "A".into(), target: "B".into(), spell: None, amount: None, aura: None, text: text.into() }
    }
    ...
    fn combat_recap_orders_newest_first() {
        let snapshot = CombatLogStatusSnapshot {
            entries: vec![
                CombatLogEntry { kind: CombatLogEventKind::Damage, source: "A".into(), target: "B".into(), spell: Some("First".into()), amount: Some(1), aura: None, text: "first".into() },
                CombatLogEntry { kind: CombatLogEventKind::Damage, source: "A".into(), target: "B".into(), spell: Some("Second".into()), amount: Some(2), aura: None, text: "second".into() },
            ],
        };
    ...
    fn collection_mounts_missing_filters_known_entries() {
        let snapshot = CollectionStatusSnapshot {
            mounts: vec![
                crate::status::CollectionMountEntry { mount_id: 1, name: "Horse".into(), known: true },
                crate::status::CollectionMountEntry { mount_id: 2, name: "Wolf".into(), known: false },
            ],
            pets: vec![],
        };
    ...
    fn profession_recipes_filters_by_text() {
        let snapshot = ProfessionStatusSnapshot {
            recipes: vec![crate::status::ProfessionRecipeEntry {
                spell_id: 100, profession: "Alchemy".into(),
                name: "Major Healing Potion".into(), craftable: true, cooldown: None,
            }],
        };
