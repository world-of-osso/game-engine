...
- [ ] Match dropdown palette/background artwork to the installed assets. Build-pinned atlas metadata now establishes the palette bounds; reference content insets and swatch layout remain under correction.
...
- [ ] Dropdown artwork remains incomplete. The old CSV mapped `charactercreate-customize-palette` to a yellow ornament; build `12.1.0.69875` metadata maps it to `[519,471..603,491]` and maps `-half` to `[729,471..813,491]`. `ui-toolkit` `78c67e7` applies those bounds. Fresh local-CASC assets matched the cache, so this was metadata drift, not stale BLP bytes. Engine native source-pixel RED/GREEN is `a55af09f`; reference content insets and dual-swatch placement still require engine work.
