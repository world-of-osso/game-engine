...
At current `8843495b`, the isolated runtime used 56.583326 of the newly authorized 60 seconds and terminated every launched process. `runtime/stretch/view.webp` visibly shows stretched native artwork with the toolkit game-menu overlay. Its UI-tree dump contains `adminvisual-proof` and 23 displayed password asterisks, not the raw dummy password. `runtime/final-login/view.webp` is black, so it cannot establish plain-login parity. This is partial rendered and semantic runtime evidence, not proof of physical input, plain-login appearance, or real authentication. Existing CPU characterization does not certify the replacement renderer. Checkboxes remain open until their full contract proof exists.

## Known gaps (current cycle)

- [ ] Cover successful plugin setup/prefill/realm/camera initialization.
- [ ] Verify plain-login artwork/layout and fade. `runtime/stretch/view.webp` proves native art plus menu overlay after Stretch; `runtime/final-login/view.webp` is black and cannot establish plain-login parity.
