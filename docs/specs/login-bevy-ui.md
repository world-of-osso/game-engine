...
- [ ] Verify native artwork/layout, modal layering, fade, corrected camera ordering, tint and image stretching through rendered proof. `runtime/menu-final/menu.webp` has opaque blocks; `runtime/tint-login/view.webp` has detached pieces. Neither is a green visual result. Menu rendering remains unresolved.
