...
- [ ] Native animation equivalence and broader scene coverage beyond the focused 67/67 integrated animation tests. Historic `a6a5d917` evidence records the former interruption jump (`x0→4→16→20`). `aaec3864`/`a495893f` retain the last Bevy-evaluated raw pose before pivot and billboard passes, then blend a stored clip to the new sequence; `interrupted-continuity/green-attempt.log` proves the focused zero-elapsed position remains continuous. Broader rotation/scale interruption coverage, GPU deformation, pixel equivalence, and real offline-scene lifecycle remain unproven. `7e3a1828`/`c52a8c88` prove real human attachment transforms and CPU shared-skin vertex deformation at two Stand samples. Walk was rejected because idle policy crossfaded it to Stand; this was a fixture-oracle confound, not a production regression. No animation performance or CPU/FPS improvement is established.
