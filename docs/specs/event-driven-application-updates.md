...
- [ ] Native animation equivalence and broader scene coverage beyond the focused 67/67 integrated animation tests. `a6a5d917` proves legacy two-pose controller timing (`x0→4→16→20`) through an interruption, not full-pose/outgoing-weight continuity: a 40% A→B blend interrupted by B→C at 30% begins B→C with B weight 70%. `7e3a1828`/`c52a8c88` prove real human assets: an equipped helm's `GlobalTransform` follows its animated joint and a shared-skin chest vertex deforms under CPU joint/inverse-bind math at two Stand samples. Walk was rejected because idle policy crossfaded it to Stand; this was a fixture-oracle confound, not a production regression. GPU deformation, pixel equivalence, and real offline-scene lifecycle remain unproven. No animation performance or CPU/FPS improvement is established.
