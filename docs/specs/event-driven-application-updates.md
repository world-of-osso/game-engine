...
- [x] Settled camera poses and unchanged M2 effect UV offsets must not emit false transform/material changes. Camera motion, collision recovery, animated UV changes, and track-removal resets must still update normally.
...
Rendering, remote interpolation, camera-facing billboards, input needed for active interaction, and active UI presentation remain render-frame-driven. Bevy's M2 pose evaluation runs in its `PostUpdate` animation stage before transform propagation; it is not a separate 60 Hz animation worker. On 2026-09-08, pre-fix current InWorld CPU observations were 306.86% and 304.85% of one core; post-fix observations were 279.27% and 281.09%. The repeat was not controlled: remote entities and clocks differ. `4cfe7bfb` removes a proven constant-pose `Changed<Transform>` invalidation, but CPU/FPS improvement is not established and the CPU goal remains unresolved.
