...
`cec56837` mirrors protocol components by canonical server entity identity. Snapshots materialize main render entities with `Remote` and required components before Player/Npc observers run; despawns clear the mapping. At shared-protocol `d3b0736` and engine `6a713af1`, `CastState` joins that snapshot set: its named spell, duration, elapsed time, cast kind, and lifecycle addition/removal cross from the worker world to the render entity. This makes real replicated cast state available to overhead rendering; it does not itself draw a spellbar. Worker and render entities are not interchangeable. `52508d1d` maps `SetTarget` outbound main entities to server identities and maps inbound emote/combat visual references back to main entities; unresolved visuals are omitted while combat logs retain server IDs. `a5f6eab0` applies the same main-to-server conversion to duel, inspect, and current/default spell targets. Explicit numeric spell selectors remain server IDs. Other entity-bit boundaries must use the same explicit conversion rather than treating world-local entity bits as equivalent.

### Authoritative NPC names

Shared-protocol `c8a06c0` makes `Npc` carry `template_id` and UTF-8 `name`; it is `Clone + Eq`, not `Copy`. Server `fc46567` loads `creature_template.name`, rejects a spawn whose template is absent rather than substituting defaults, and copies that name into each spawned `Npc`. Engine `79a252be` clones `Npc` into replication snapshots, so the main-world `On<Add, Npc>` observer receives the captured name with its support components and later snapshots can replace it. The UTF-8 fixture `Loup — Écorché` covers the SQLite-to-spawn and worker-to-main snapshot boundaries. This documents data availability only: UI projection and visible nameplate rendering remain separate work.

`runtime-tests-transport.log` records 13/13 module tests, including encoded Lightyear loopback delivery while the main app remains unupdated. `owned-inbox-red-behavior.log` then `owned-inbox-green.log` record dispatcher RED and 8/8 GREEN proof. `independent-udp-handshake.log` records a real UDP handshake **1/1** after `54411453` excluded resource entities from the replication `EntityRef` query; the worker sent handshake packets without a main-app update. `worker-restart-tests.log` records **3/3**: worker join, closed old sender, stale queue cleanup, then a second UDP handshake without a main-app update. `worker-char-create-response.log` records **3/3** real transport response tests through the separate worker App and main inbox relay; `worker-auth.log` records **23/23**; `wire-identity.log` records **19/19**. `migrated-ui-reconnect-fixtures.log` records **11/11**, including the previously pending reconnect fixture. `worker-permit-lifecycle-fixture-green-retry.log` records **3/3** worker-permit lifecycle/cadence cases; active cooldown coverage is **3/3**. Native `worker-native-inworld/` reached InWorld with player/NPC mirrors and a routed Who reply (`Theron`, one result). Replicated equipment set/clear is separately proven; native ordinary disconnect/reconnect proof is recorded below.
...
## Cast presentation boundary

Server `211d86a`/`3ba4085` adds the currently bounded authoritative cast lifecycle. Player `SpellCastIntent` and `StopSpellCast` messages now enter a server handler; available spell data is validated, a named `CastState` is attached for non-instant casts, elapsed time advances, and state is removed on stop, forbidden movement, or expiry. The handler intentionally does not apply spell effects, mutate health, or create NPC casts. Targeted server tests passed for start/progress/expiry, stop/movement cancellation, and rejected requests. The corresponding engine snapshot test is written but has no current passing engine command record; overhead spellbar rendering is separate in-progress work.

## Current proof and limits

Focused proof records historical main-thread network **8/8**, migrated API **55/55**, equipment **13 passed** plus **2 appearance-event tests**, equipment IPC FIFO **1/1**, and worker character-create transport responses **3/3**. New worker proof records worker/adapter **13/13**, owned-inbox dispatcher **8/8**, restart cleanup **3/3**, worker-permit lifecycle/cadence **3/3**, worker auth **23/23**, wire identity **19/19**, migrated UI/reconnect fixtures **11/11**, sound maintenance **39/39**, and active cooldown coverage **3/3**. `4d3c7ed6` adds actual-worker RED/GREEN proof that a forced notice preserves its policy data and requests the worker Netcode disconnect. At `1cce4171`, an authenticated native InWorld client matched the server connection; an admin kick then produced real `Disconnected`, InWorld → Login, visible `LoginRoot`, and zero network links/replicas. Hidden semantic scene entries remain, so this is not full scene cleanup or visual proof. Client `1057253` also completed an ordinary native reconnect: test-server PID `483229` restarted as PID `1060054`, the client timed out, connected with a new ID, reauthenticated, returned to InWorld, and routed `Who Theron` with one result. The test client was stopped; the server remains running. Native Head 1128 set/clear proves rig/model retention and baseline appearance restoration. `7e3a1828`/`c52a8c88` add two real-human asset proofs: an equipped helm's `GlobalTransform` follows the sampled animated joint, and a shared-skin chest vertex changes under CPU joint/inverse-bind deformation at two Stand samples. `f616f48a` proves the real CharSelect asset lifecycle: human + helm spawn and animate, root/joints/helm despawn, then respawn and animate again. Walk sampling was rejected because idle policy crossfaded it back to Stand, invalidating the fixture oracle rather than exposing a production regression. Remaining proof is native visual equivalence and controlled CPU/performance measurement; GPU deformation and pixel equivalence are not inferred from CPU-side skinning.

## Sources

- [`src/network_runtime/replication.rs`](../../../src/network_runtime/replication.rs) — engine worker/main cast snapshot bridge
- [`../../../shared-protocol/src/casting.rs`](../../../../shared-protocol/src/casting.rs) — replicated cast state
- [`../../../game-server/crates/server/src/cast_presentation.rs`](../../../../game-server/crates/server/src/cast_presentation.rs) — bounded server cast lifecycle
...
**Bounded startup proof (2026-09-09)**: before server commit `34e7551`, local `cargo rd --bin game-engine -- --no-ui --screen inworld` captured the main thread at three seconds in `insert_npc_transform` → `spawn_replicated_npc` → uncached `load_m2_cached` → `parse_bone_animations`; `apply_network_updates` precedes `run_network_ticks` in `First`, so premature NPC replication blocked the first main-world auth dispatch. `34e7551` hides existing and newly spawned `NetworkVisibility` targets for every `ClientVisibility` connection until character selection grants nearby targets. Its verifier records 25 focused tests plus formatting and check. The exact command then reached login success at app time 1.896 s and InWorld at 6.57 s within the ten-second limit (`data/diagnostics/northshire-appearance-placement/{proof-ledger.md,post-visibility-run.txt}`). A standalone screenshot CLI attempt lacked `LD_LIBRARY_PATH`; this is startup proof, not visual proof for the original NPC appearance or cart defects. The historical 18-second delay was a different invocation.
...
See [authentication storage](../../authentication.md#storage) for server-keyed engine token paths and the separate headless CLI cache.
...
- [replication snapshot source](../../../src/network_runtime/replication.rs) — cloned `Npc` snapshots reach main-world add observers
- [shared `Npc` component](../../../../shared-protocol/src/components.rs) — replicated template ID and authoritative UTF-8 name
- [server NPC spawn source](../../../../game-server/crates/server/src/zones.rs) — copies loaded template name into spawned NPCs
