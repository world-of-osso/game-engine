...
`NetworkTick` remains a main-world logical application cadence, but live Lightyear work no longer runs there. Each connection owns a separately clocked 60 Hz network worker, so UDP, transport timers, replication, and typed message receive continue while rendering or the main world is blocked. The authoritative client/server simulation remains negotiated at 20 Hz; the worker changes neither wire formats nor simulation rate.
...
Focused proof records historical main-thread network **8/8**, migrated API **55/55**, equipment **13 passed** plus **2 appearance-event tests**, equipment IPC FIFO **1/1**, and worker character-create transport responses **3/3**. New worker proof records worker/adapter **13/13**, owned-inbox dispatcher **8/8**, restart cleanup **3/3**, worker auth **23/23**, wire identity **19/19**, and migrated UI/reconnect fixtures **11/11**. A native client reached InWorld and mirrored player/NPC state, but this does not prove complete replicated component/equipment lifecycle, clean appearance delivery, or idle-CPU improvement.
