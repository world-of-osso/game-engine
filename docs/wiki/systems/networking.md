...
`cec56837` mirrors the 15 protocol components by canonical server entity identity. Snapshots materialize main render entities with `Remote` and required components before Player/Npc observers run; despawns clear the mapping. Worker and render entities are not interchangeable. Wire fields containing entity bits still need explicit server-identity ↔ render-entity conversion at target, duel, inspect, spell, emote, combat, and related UI boundaries.

`runtime-tests-transport.log` records 13/13 module tests, including encoded Lightyear loopback delivery while the main app remains unupdated. `owned-inbox-red-behavior.log` then `owned-inbox-green.log` record dispatcher RED and 8/8 GREEN proof. Latest focused runtime evidence is 19/20, not an integrated green result: a UDP run exposed Bevy B0002 because replication forwarding queried resource entities as `EntityRef`. `54411453` excludes resources, but it has not yet received fresh full integration, reconnect, binary-fixture, or native proof. This establishes implementation structure, not completed render-independent networking or CPU improvement.
...
Focused proof records historical main-thread network **8/8**, migrated API **55/55**, equipment **13 passed** plus **2 appearance-event tests**, equipment IPC FIFO **1/1**, and character-create response **3/3**. New module proof records worker/adapter **13/13** and owned-inbox dispatcher **8/8**. The integrated runtime suite is currently **19/20**, with the B0002 fix awaiting a fresh run. It does not prove complete connected/reconnect lifecycle, binary fixtures, native appearance delivery, or an idle-CPU improvement.
