...
At engine `ae941bac`, revision-scoped compiler-emitted binaries passed 57 distinct focused cases: 35 bin login cases, 16 library automation/native/IPC cases, and 6 presentation cases. The presentation scope covers status replacement, masked password display, focus/fade, button states and hidden controls. Subsequent setup, startup, and tint proof totals 64 focused cases. Evidence records commands, executable identities and retained failed attempts in `data/diagnostics/login-bevy-ui/verification/`. `runtime/live-auth/prefilled/` proves the tested local dev-login path: the native ConnectButton transitions Login to Connecting, sends the prefilled `admin` credentials, receives Login success with two characters, reaches CharSelect, dumps its UI tree, and emits AppExit::Success. The enclosing nine-second timeout returns `-9` during shutdown, so this is not clean-exit proof. The earlier `alice` attempt timed out before submission and is retained as non-passing evidence. Physical input and exact baseline-pixel parity remain open.
