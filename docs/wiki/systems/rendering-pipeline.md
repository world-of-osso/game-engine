...
- **Empty-stage camera bundle cause (confirmed)**: diagnostic commit `b6468868` removed only the WowCamera TAA/SSAO/depth/normal/motion-vector prepass bundle and temporal jitter/mip-bias support while preserving the performance overlay and other camera behavior. Alessio judged the connected Empty client materially improved, confirming this bundle as the cause; six post-warmup samples in `/tmp/claude/game-engine-perf/empty-camera-post-process-live.json` are supporting only. Commit `e9d3d470` removed the temporary selector from source. The preserved diagnostic client remains live as PID `3367453`; no permanent stage-aware fix is implemented yet.
