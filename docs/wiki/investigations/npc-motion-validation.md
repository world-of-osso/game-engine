...
The prior Loading→InWorld regression only proved that roots parented to HumanHD attachment bones; it did not establish their semantic hand placement. Local WMVx reference semantics are right palm 1 (HumanMaleHD bone 206), left palm 2 (bone 211), and shield left wrist 0 (bone 201). Commit `496a057b` corrects the engine's former main-hand 0/off-hand 1 mapping. Its live sword/shield test was RED against the old mapping and passes at `5e5b2574`; a one-time approved 59.173-second native run adds rear/side visual support for corrected placement. The capture turns actor yaw rather than the camera, so its filenames do not establish exact viewing angles. The earlier absolute-path guard diagnosis still applies to SKA1 attachment loading, not this semantic mapping correction.
