...
[Shared plugin frame ordering](../design/ui-frame-order.md) is approved for documentation only. It would consolidate six sorts while retaining standalone setup through explicit wrappers; plugin integrations would order against named sets. No implementation or performance evidence exists for that design yet.
