...
The first flat profile also exposed a separate transform-worker queue defect: 90.05% of samples inside `propagate_parent_transforms` worker polling were on the receiver lock-retry branch. A plain blocking-mutex experiment measured 368.72%/368.83% and was reverted; it left the empty-queue/active-worker loop hot. `3f31ad07` instead replaces that loop with blocking batch receive plus completion wake batches. Its RED burns 29 thread CPU ticks in 300 ms; GREEN is 1/1. All vendored transform tests are 12/12, format and dev check pass. In the same settled one-tile workload with **84 remote entities** before and after, two post-animation pre-queue captures average **280.18%**, while two post-queue captures average **246.21%**: a **12.13% directional reduction**. Separate process runs and host clock/load variance remain, but this is stronger comparable CPU evidence than the earlier differing-remote observations. CPU is still excessive; goal remains unresolved. Artifacts: `data/diagnostics/cpu-goal-resumed/{current-baseline,transform-dirty,transform-queue-design}`.
