...
`56e04991` replaces wake tokens with a condition variable and mutex-protected pending batches/active-producer count. Workers sleep only while real work remains active and no batch is available; checked-out batches retire through a drop guard. Both targeted regressions pass: repeated propagation updates the child to 23, and an idle waiter uses at most two CPU ticks during 300 ms. Full current verification and native CPU comparison remain pending; the CPU goal remains unresolved. Evidence: `data/diagnostics/cpu-goal-resumed/transform-lock/{sentinel-falsification,condition-green.log}`.
