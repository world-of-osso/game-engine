...
Raw post-fix observations are **279.27%** and **281.09%** over 20 seconds, versus pre-fix observations of **306.86%** and **304.85%**. They share server, character, tile state, binary configuration, and 40 threads, but remote entities differ (83 before; 84/85 after), clock ranges are uncontrolled, and the repeat was not a controlled comparison. Record the observations without causal interpretation; the excessive-CPU goal remains unresolved. Artifacts: `data/diagnostics/cpu-goal-resumed/{current-baseline,transform-dirty}`.
