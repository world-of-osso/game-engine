...
Borrowing follow-ups `6802940` and `66e6d35` remove eager source-text copies without caches; `1050abb` also removes one repeated visible-frame size read and uses the existing total comparator with unstable sorting. See the [UI record](../systems/ui-system.md). Existing shadow cases and five main-text cases pass before/after, including two new Unicode/password cases; frame ordering has three before/after cases (one new). This adds three distinct tests to the batch above, not another eleven. [Shadow build integration](../../../data/diagnostics/unchanged-writes-20260909/shadow-borrow/verification/final-integration-report.md), [main-text compilation integration](../../../data/diagnostics/unchanged-writes-20260909/text-borrow/verification/final-integration-report.md), and [frame-order verification](../../../data/diagnostics/unchanged-writes-20260909/frame-order/verification/report.md) retain their provenance limits. Password-mask generation and helper collections still allocate; no allocation-count or CPU measurement was performed.

[Shared plugin frame ordering](../design/ui-frame-order.md) is implemented at toolkit `02a3049`, tested through `3e61227`. One plugin preparation replaces six local builds; standalone entry points remain fresh and plugin ordering uses named sets. [Independent verification](../../../data/diagnostics/unchanged-writes-20260910/shared-frame-order/verification/report.md) records 39 passing cases across seven focused targets, toolkit checks/readability and bounded engine compilation. The one-build/map bound is source-audited, not a CPU or internal-call-count measurement. Existing outline reconciliation limits and supplied engine provenance are retained in that report.
...
An 8-second 99 Hz `cpu-clock` sample makes Bevy transform parent propagation the largest individual flat symbol at **19.46%** of samples, but does not establish it as the whole-process root cause. It established a concrete upstream invalidation defect: constant animated raw TRS poses produced three downstream `Changed<Transform>` notifications. `4cfe7bfb` redirects Bevy raw-TRS evaluation into `RawBonePose` and writes the pivot-corrected `Transform` with `set_if_neq`; RED reproduces the three notifications and GREEN is **1/1**.

Raw post-fix observations are **279.27%** and **281.09%** over 20 seconds, versus pre-fix observations of **306.86%** and **304.85%**. They share server, character, tile state, binary configuration, and 40 threads, but remote entities differ (83 before; 84/85 after), clock ranges are uncontrolled, and the repeat was not a controlled comparison.

The first flat profile also exposed a separate transform-worker queue defect: 90.05% of samples inside `propagate_parent_transforms` worker polling were on the receiver lock-retry branch. A plain blocking-mutex experiment still measured 368.72% CPU and was reverted. The subsequent `3f31ad07` completion-wake-token approach recorded lower CPU, but `91953625` falsified its correctness: a later propagation pass left child translation at 13 instead of 23. Those CPU observations are not valid optimization proof because work could be skipped.

`56e04991` replaces wake tokens with a condition variable and mutex-protected pending batches/active-producer count. Workers sleep only while real work remains active and no batch is available; checked-out batches retire through a drop guard. Independent verification passes formatting/checks, 13 crate tests and 5 doctests, including repeated propagation and low-CPU waiting.

A coordinated native pair using Theron, one loaded tile, 84 remote entities and 40 threads measured **299.285% → 243.439% CPU** over 20 seconds each; mean sampled CPU clocks were **2.770 → 2.796 GHz**. The 18.66% reduction supports bounded improvement in this workload, not a universal causal estimate. A later patched sample measured 277.013% while mean clocks fell to 0.838 GHz; it is noncomparable, not confirmation. No FPS measurements or caps were used. The CPU goal remains unresolved. Evidence: `data/diagnostics/cpu-goal-resumed/{controlled,transform-lock/final-verification}`.
...
### Achievement-frame idle-state follow-up

A direct sampled achievement-category leaf led to source confirmation of redundant row construction during settled InWorld; partially recovered caller frames were not treated as proof of allocator ownership. `sync_achievement_frame_state` rebuilt static tabs/categories/rows every Update before comparing the resulting model, including when hidden. `e26393c9` returns before construction unless `AchievementCompletionState` or `AchievementFrameOpen` changed; the pre-existing equality check remains. Four registered-system behavioral tests cover unchanged open state, opening, closed progress update, and reset/lifecycle behavior. Cached-system characterization: median **2990.58→795.87 ns/sync** across7×250k sequential runs, UI setup/warm-up excluded. This is local timing characterization, not RED or native CPU proof. Behavior/fmt/check verification passed; `a173c56d` names the equivalent input-change condition to address a readability finding. Timing belongs to `e26393c9`, not a whole-client CPU measurement. The local saving is microsecond-scale; no native gain quantified. Proof: `data/diagnostics/cpu-goal-resumed/achievement-idle/`.

### Current function-level profile after retained fixes

An eight-second user-space CPU capture of `71535eed` recorded **1,642 samples, zero lost**, with Theron,82→83 remotes, and one Azeroth32,48 tile verified before capture. Largest named leaf: `bevy_pbr::render::skin::extract_skins`, **31 samples (1.89%)**. No single sampled function dominates. Initial caller export had1,596 empty blocks. All46 framed samples belonged to the first sampled thread, matching the [upstream perf/libdw thread-cache bug](https://lkml.iu.edu/hypermail/linux/kernel/2607.3/01350.html). User-authorized per-TID replay of the same capture recovered1,114 leading frames and637 multi-frame samples without recapture or host changes; independent verification preserves all1,642 event identities and matches every recovered leading IP. Parent accuracy remains unverified (unknown/invalid-address frames remain), so generic helper costs still cannot be assigned reliable subsystem totals. This diagnostic workaround retires after a fixed perf passes unfiltered multi-thread replay. A separately extracted, signature-verified Arch perf7.2.4 package still produced only46 framed samples on the same capture; it was not installed and no system tool was replaced.
...
`bd244365` updates animation time only when any layer has nonzero XY UV velocity and the time differs. Shader time is consumed only by that velocity multiplication; reflection uses a separate Z component. Mixed static/moving regression emitted both asset IDs before the fix and only the moving ID afterward. Six targeted tests pass, including unchanged time, environment replacement/absence, and animated freeze fixtures. Independent verification passed at `bd244365`; subsequent native comparisons did not justify a CPU-gain claim. First baseline/patched pair232.936%→306.977%, clocks2.776→2.198GHz; repeat233.133%→309.729%, clocks2.811→2.045GHz. `ee1bd46a` withdraws time filtering to isolate this repeated increase; environment synchronization remains. Six-test proof is historical, not proof that time filtering remains current. Environment-only `ee1bd46a` measured236.928% at2.366GHz,81→82 remotes, versus repeat baseline233.133% at2.811GHz,81 remotes. This returns near baseline process CPU but still establishes no isolated gain or explanation for the combined increase. All five captures retained Theron on one Azeroth32,48 tile; owned clients stopped. User-authorized paired throughput follow-up recorded baseline239.258% CPU /33.5225 mean smoothed application FPS versus combined310.903% /41.2725. Approximate CPU per application update71.372→75.329ms; clocks3.059→2.151GHz, both84 remotes. IPC FPS comes from Bevy `Time<Real>` diagnostics, not a GPU-present counter: these are throughput proxies, not exact CPU per rendered frame. Higher throughput is observed; lower CPU cost is not established. Time filtering remains withdrawn. Proof artifacts: `data/diagnostics/cpu-goal-resumed/terrain-invalidation/`.
...
Render `Prepare` removal changed **340.170% → 190.186%** but halved cadence **226.447 → 114.746/s** and worsened CPU/update; it is deliberately not interpreted as a saving. Removing `Specialize` produced no reduction (**335.071% → 343.169%**). The `Queue` split remains unparsed. The historical true Empty run failed before sampling because its configuration disabled `LightPlugin`, removing required `PointLightShadowMap` and scattering-medium providers. `be9a1ff7` keeps LightPlugin registered while Empty has no light entities; native GREEN remains pending under `update-schedule-isolation/strict-empty-fixed/`.
