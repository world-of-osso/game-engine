...
Recovered leaf/caller evidence found achievement-row allocation and destruction during settled InWorld. `sync_achievement_frame_state` rebuilt static tabs/categories/rows every Update before comparing the resulting model, including when hidden. `e26393c9` returns before construction unless `AchievementCompletionState` or `AchievementFrameOpen` changed; the pre-existing equality check remains. Four registered-system behavioral tests cover unchanged open state, opening, closed progress update, and reset/lifecycle behavior. Cached-system characterization: median **2990.58→795.87 ns/sync** across7×250k sequential runs, UI setup/warm-up excluded. This is local timing characterization, not RED or native CPU proof. Independent verification and native pair pending. Proof: `data/diagnostics/cpu-goal-resumed/achievement-idle/`.

### Current function-level profile after retained fixes

An eight-second user-space CPU capture of `71535eed` recorded **1,642 samples, zero lost**, with Theron,82→83 remotes, and one Azeroth32,48 tile verified before capture. Largest named leaf: `bevy_pbr::render::skin::extract_skins`, **31 samples (1.89%)**. No single sampled function dominates. Initial caller export had1,596 empty blocks. All46 framed samples belonged to the first sampled thread, matching the [upstream perf/libdw thread-cache bug](https://lkml.iu.edu/hypermail/linux/kernel/2607.3/01350.html). User-authorized per-TID replay of the same capture recovered1,114 leading frames and637 multi-frame samples without recapture or host changes; independent verification preserves all1,642 event identities and matches every recovered leading IP. Parent accuracy remains unverified (unknown/invalid-address frames remain), so generic helper costs still cannot be assigned reliable subsystem totals. This diagnostic workaround retires after a fixed perf passes unfiltered multi-thread replay.
