...
This proves cache reuse in the shared replicated spawn path. Subsequent connected startup recovery, restored FPS updates, and route measurements are recorded in [[movement-performance]]; they do not establish a general rendering-speed improvement.

For repeatable non-teleport movement, commits `15928f4e`, `d3f525ac`, and `6c9d82a3` add a bounded timed-forward state consumed by the ordinary `player_movement` path. It preserves normal speed, collision, animation direction, and network input while cancelling on manual input, modal/reconnect state, and world exit. Existing `map waypoint add` attempts at the current spawn did not displace the player, including short nearby targets, so earlier waypoint samples are invalid as moving-frame evidence. Later collision-free routes supplied connected displacement, a stationary/moving comparison, and a tile-boundary stall reproduction; see [[movement-performance]]. See [[scripted-movement]] for controls.
...
The initial reconnect storm had a separate lifecycle cause: Lightyear's required reasonless `Disconnected` marker was mistaken for a real InWorld loss during `PendingConnect`. Commit `0d215316` fixes that boundary; its real App-level RED/GREEN ordering proof is recorded in `/tmp/claude/game-engine/reconnect-initial-marker-red.log` and `/tmp/claude/game-engine/reconnect-initial-marker-green.log` (**14 passed**). Post-build proof in `/tmp/claude/game-engine/reconnect-fixed-runtime.json` and `/tmp/claude/game-engine/reconnect-fixed-live-current.log` used engine docs commit `6b034959`: old PID `2846177`/SHA `84f6ecc...` was terminated first, then fixed PID `3715288`/start ticks `192167263`/SHA `aef6f08d318a21063d698816b8202ed21f6ca70b7cf4d015a2520a5f107619c3` held one unchanged client ID for ten seconds. It stayed `InWorld` with `connected=true`, `connected_links=1`, `local_players=1`, ping `pong`, and **10.21 FPS / 97.98 ms** at `focused=false`; 78 NPC entries were all undisplayed, camera/terrain/WMO/doodad/particle terms were absent, terrain/cache counts were zero, and the log had one expected initial Connecting marker, one connect/login/InWorld path, and zero InWorld disconnects, reconnect loop, panic, OOM, or device-loss messages. Server PID `3123827` remained cargo-watch managed with admin ping `pong` and server HEAD `4aca4d3` containing the Who fix. This proves stable rebuilt runtime health without another server restart. Account state was healthy; no stale-session or token explanation was established. An earlier direct M2 launch ended at a Friz font parse failure; the font files were overwritten at 18:23 UTC, the pre-overwrite bytes were not preserved, and the current Friz/Arial bytes pass the exact Bevy parser. The initial paired run then used the same binary/source/options/server/token/environment, ten readiness polls, one local player, stable recorded world/material invariants, 125-second holds, and six unprofiled samples. Enabled measured **31.767 FPS / 54.177 ms**; disabled measured **36.843 FPS / 54.482 ms**. However, the first performance sample in each condition immediately followed an expensive `dump-scene` request and inherited its long diagnostic frame (**188.06 ms** enabled after **215 ms** scene latency; **210.88 ms** disabled after **266 ms** scene latency). The recorded readiness workloads also differed (**135** versus **133** remote entities). Therefore this initial comparison is **preliminary/inconclusive pending a clean repeat** with a prospective performance warm-up; it supports no M2 performance conclusion or fix. Selector/tests/flag were removed in `58e2f9c2`, then restored temporarily in `a7784e70` for the clean repeat.
...
## Strict Empty Pacing Retirement

Commit `281d291a` removes `4fb2e5c9`'s forced 100 ms / 10 FPS strict-Empty limiter per the explicit no-FPS-caps requirement. Exact Empty now uses only the configured global `GraphicsOptions` frame-rate limit; with that limit disabled, Empty is uncapped. Networking, IPC, and the FPS overlay remain registered.

All earlier strict-Empty results near 10 FPS—including the **11.199%** reading at approximately **9.994 updates/s**—are historical capped measurements. They are neither uncapped CPU baselines nor evidence of a CPU fix, and must not be converted into a 97% CPU-gain claim. The successful startup boundary from `35e15d27` is separate: Empty is startup-only and omits PBR/light/debug/material pipelines plus target visuals; it retains no same-process return to full rendering. An uncapped Green main run remains pending.
...
Commit `53a9f66a` adds an environment-gated movement timing probe in `src/rendering/camera/camera.rs`. Set a nonempty `WOO_PERF_MOVEMENT` value to emit once-per-second stderr summaries for `player_movement`, including frame, waypoint-pathing, and collision average/max durations plus WMO/doodad collider counts; unset the variable to keep the probe inactive. This records diagnostic timing only and has no runtime performance conclusion until a controlled live measurement exists.
...
- `/tmp/claude/game-engine/reconnect-fixed-runtime.json` and `reconnect-fixed-live-current.log` — fixed rebuilt-client stability and strict-Empty health proof
