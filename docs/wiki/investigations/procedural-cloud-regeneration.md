...
For repeatable non-teleport movement, commits `15928f4e`, `d3f525ac`, and `6c9d82a3` add a bounded timed-forward state consumed by the ordinary `player_movement` path. It preserves normal speed, collision, animation direction, and network input while cancelling on manual input, modal/reconnect state, and world exit. Existing `map waypoint add` attempts at the current spawn did not displace the player, including short nearby targets, so earlier waypoint samples are invalid as moving-frame evidence. A later collision-free route supplied connected displacement and a stationary/moving comparison; see [[movement-performance]]. Tile-boundary behavior remains unmeasured. See [[scripted-movement]] for controls.
