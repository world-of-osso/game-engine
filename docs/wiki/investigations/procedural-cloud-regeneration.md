...
For repeatable non-teleport movement, commits `15928f4e`, `d3f525ac`, and `6c9d82a3` add a bounded timed-forward state consumed by the ordinary `player_movement` path. It preserves normal speed, collision, animation direction, and network input while cancelling on manual input, modal/reconnect state, and world exit. Existing `map waypoint add` attempts at the current spawn did not displace the player, including short nearby targets, so earlier waypoint samples are invalid as moving-frame evidence. The new route still needs a connected IPC/CLI displacement proof before stationary, moving, or tile-boundary measurements can be compared. See [[scripted-movement]].
