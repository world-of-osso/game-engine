...
The matching libc build ID (`503200d7fda94a5dc6058d7e0694e5d1dcb2e372`) was present in the preserved perf metadata and resolved with an 11 MB debug artifact downloaded from Arch debuginfod into the diagnostic data. libc covers 1,413,726,253 sampled period (12.1680% of the original total). One-address-at-a-time symbolization resolves `memcpy` to 2.4163%, `memset` to 1.1201%, `_int_free_chunk` to 1.7458%, and `_int_malloc` to 1.3708%. These are shares of sampled cycle period, not CPU percentage points. They establish allocation, freeing, copying, and clearing work, but cannot charge those operations to task lifecycle or a specific render callback without runtime ancestry. Independent verification of this library attribution remains pending.
