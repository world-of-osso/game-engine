...
Commit `55a7abe9` additionally selects Bevy's existing `producer_mark_dirty`, `consumer_mark_dirty`, and `par_traversal_mark_dirty` worker spans. Test `5712ae71` first proved they were absent, then proved positive CPU export after selection. This measures worker task polling independently; it does not charge worker CPU to `mark_dirty_trees`, count distinct tasks, or establish the source candidate as the bulk cause. Bevy source shows `mark_dirty_trees` creates its consumer and traversal workers before scanning changed transforms; native capture is pending.
